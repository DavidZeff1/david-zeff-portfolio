...
import { caseStudies, softwareProjects, skillGroups } from "@/lib/projects";
          ...
          push(
            <div className="space-y-2 line-in">
              <div style={{ color: t.dim }}>
                analytics/ ({caseStudies.length}) — try `analyst` for the findings
              </div>
              {caseStudies.map((p, i) => (
        ...
        case "data":
          push(
            <div className="space-y-3 max-w-2xl line-in">
              <div style={{ color: t.dim }}>
                // {caseStudies.length} end-to-end analyses — each leads with what it found
              </div>
              {caseStudies.map((p) => (
